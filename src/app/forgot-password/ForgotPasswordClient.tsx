...
export default function ForgotPasswordClient() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
        redirectTo: `${window.location.origin}/reset-password?redirect=${encodeURIComponent(redirectTo)}`,
      });
  ...
  async function onVerifyAndReset(e: FormEvent) {
    ...
    try {
      ...
      const r = await getRoleForUser(updated.user.id);
      const dashboardHref =
        r === "admin"
          ? "/dashboard/admin"
          : r === "broker"
  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-md items-center px-4 py-10">
      <div className="w-full rounded-3xl border border-black/10 bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-6 text-xl font-semibold">Mot de passe oublié</h1>
        ...
        {sent ? (
          <form onSubmit={onVerifyAndReset} className="mt-6 space-y-3">
            <div>
            ...
            <div>
              <label className="text-xs font-medium text-zinc-700">Code de confirmation</label>
              <input
                required
                value={otp}
              ...
              <div className="mt-2 text-xs text-zinc-600">
                Tu peux aussi utiliser le lien reçu par email si ton email n’affiche pas de code.
              </div>
            </div>
